...
const tasks = {
            boy:["Съесть сырую картошку!", "Потанцевать!", "Кукарекать 30 секунд!", "Стоять на голове!", "Приготовить обед!", "Скорчить рожицу!","Пошутить!","Помыть посуду!","Сделать мем!","Говорить минуту смешным голосом!"],
            girl:["Съесть сырую картошку", "Потанцевать", "Кукарекать 30 секунд", "Стоять на голове", "Приготовить обед", "Скорчить рожицу","Пошутить","Помыть посуду","Сделать мем","Говорить минуту смешным голосом"]
        }
...
const playerButtons =  Array.from(document.getElementById("player-choise").children);
...
homeButtons.forEach((el) => {
    ...
    el.addEventListener("click",() => {openPage(parent, page1); resetGame()});
})
...
function updateTasks(event) {
    ...
    (player === "girl") ? currentPlayer.innerHTML = "ОНА:" : currentPlayer.innerHTML = "ОН:";
    punishBlock.style.display = 'none';
